...
### Added
- `bin/chrome-mcp-launch.sh`: a per-workspace launcher for the Chrome DevTools
  MCP (`chrome-devtools-mcp`). Register it once, user-scoped —
  `claude mcp add chrome -s user -- <repo>/bin/chrome-mcp-launch.sh` — and every
  workspace gets its OWN browser instead of sharing one. The launcher reads its
  real launch directory (`pwd -P`, never `$PWD` — an IDE can leak a stale one
  from another window), derives the workspace slug, and hands Chrome a
  persistent per-slug profile (`~/.chrome-mcp/<slug>`), so agents in different
  workspaces drive separate, isolated browsers — logins preserved — from a
  single registration, with nothing for the agent to choose (the profile follows
  the directory you run `claude` in). This routes around Claude Code collapsing a
  git worktree onto its main repo for MCP config, which makes per-worktree
  registration impossible. Each window is tinted to its workspace's accent color
  (the `titleBar.activeBackground` hex, via Chrome's
  `--install-autogenerated-theme`, which survives the automation launch's
  `--disable-extensions`), so the browsers are distinguishable at a glance.
  Outside a workspace it falls back to a shared `_default` profile. Requires the
  `claude` CLI; remove the old shared `chrome-devtools` server once this is set
  up so a stray extra browser doesn't linger.
...
### Added
- `MR_DRAFT` config (default `true`) controls whether `ws mr` creates merge
  requests as drafts. With `false`, MRs are created ready for review. The new
  `ws mr --draft` forces a draft and `ws mr --nd` forces no-draft, each
  overriding the config for that run (giving both is an error).
