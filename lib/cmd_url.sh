...
# Open Terminal on a generated .command file instead of driving it with
# AppleScript (open_terminal_window's `tell application "Terminal"`).
#
# Both routes end in a Terminal window running the command, but the AppleScript
# one is an Apple Event, and an Apple Event from the handler app needs the user
# to grant Automation consent in a modal dialog the FIRST time a link is
# followed — while the link itself appears to do nothing. Worse, the failure is
# invisible: url_notify's fallback is also an Apple Event, so a denied consent
# silences the error too. `open` is a Launch Services call, needs no consent,
# and works on a machine where nothing has been granted yet.
url_open_command_window() {
  local cmd="$1" name="$2" dir file
  dir="${TMPDIR:-/tmp}/ws-url"
  mkdir -p "$dir" || return 1
  # One file per verb+slug, not per click: re-following the same link reuses its
  # script rather than growing a pile of them.
  file="$dir/$(printf '%s' "$name" | tr -c '[:alnum:]._-' '-').command"
  {
    printf '#!/bin/bash\n'
    printf '# Generated by `ws url` for a %s:// link. Safe to delete.\n' "$WSM_URL_SCHEME"
    # Echo the command first, so the window shows what it is running. A quoted
    # heredoc, because $cmd is full of the single quotes url_shquote added.
    printf "cat <<'WSURLCMD'\n$ %s\nWSURLCMD\n" "$cmd"
    printf '%s\n' "$cmd"
    # Hand the window to an interactive shell afterwards, the way an AppleScript
    # `do script` does. Terminal runs a .command with a trailing `exit`, so
    # without this the shell closes the moment the command finishes — and with
    # "close the window on exit" configured, its output goes with it.
    printf 'exec "${SHELL:-/bin/zsh}" -l\n'
  } >"$file" || return 1
  chmod +x "$file" || return 1
  vlog "Opening Terminal on $file"
